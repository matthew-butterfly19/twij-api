...
exports.scheduleTest = async (newTest) => {
  ...
  const emailsOptions = newTest.emails.map(email => {
    ...
    return {
      ...
      <p>Data rozpoczęcia quizu: <b>${moment(newTest.startTime).add(1, 'hours').format('DD.MM.YYYY HH:mm')}</b></p>
      <p>Quiz będzie dostępny do godziny: <b>${moment(newTest.startTimeEnd).add(1, 'hours').format('DD.MM.YYYY HH:mm')}</b></p>
